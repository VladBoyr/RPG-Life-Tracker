...
start "Django Server" /D "%BACKEND_PATH%" cmd /K "call .\venv\Scripts\activate.bat && echo Backend environment activated. Starting server... && pip install -r requirements.txt && coverage run --source='.' manage.py runserver && coverage report"
...
start "Playwright Tests" /D "%FRONTEND_PATH%" cmd /K "npx playwright test && npx playwright show-report && pause"
